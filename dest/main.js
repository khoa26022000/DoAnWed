...
function addActiveMenu(){
   ...
   sections.forEach(function(section,index){
        if(scrollSection>(section.offsetTop-header.clientHeight) && scrollSection<=(section.offsetTop+section.clientHeight))
        {
            removeAtiveMenu();
...
function addBackToTop(){
    let scrollY=window.pageYOffset;
    if(scrollY >(sliderHeight- header.clientHeight) && scrollY <= (footerHeight-header.clientHeight-50))
    {
...
let textfile=$('.textfile'),
    ...
    inputfile.on('change',function(){
        textfile.html($(this).val().match(/[^\\/]*$/)[0]);
    })
    // form logo
let logo=$('.logo')
let dong=$('.close')
...
function click(){
    ...
    accordion.click(function(e){
        ...
        $(this).closest('.accordion').siblings('.active').removeClass('active').find('.accordion__text').stop().slideUp(200);
    })
}
document.addEventListener('click',function(){
...
window.addEventListener('resize', function(){
   ...
   $(".flickity-viewport").css('height',document.querySelector('.slider-wrap').clientHeight);
})
// news
